...
void Primary::singlePendulum() {
  ...
  Body *b1 = builder.createBody().position(0, 0, 5).fixed(true).build();

  Body *b2 = builder.createBody().position(1, 6, 5).build();
...
void Primary::doublePendulum() {
  ...
  Body *b2 = builder.createBody().position(5, 0, 5).build();

  Body *b3 = builder.createBody().position(5, 0, 10).build();
...
bool Primary::load() {
  ...
  // FIXME: Full path too files are temporary because of move from Windows to
  // Linux development!
  if (!m_ctx.renderer->getShaderManager().loadShader(
          "bodyShader",
...
void Primary::update(const double dt) {
  ...
  if (m_ctx.input->isKeyPressed(GLFW_KEY_SPACE)) {
    toggle(m_run);
  }
...
void Primary::render() {
  ...
  m_systemVisualizer.render(m_system, m_camera.getPosition(),
...
void Primary::showUI() {
  ...
  const int milliseconds =
      static_cast<int>((totalSeconds - std::floor(totalSeconds)) * 1000);
  ...
  ImGui::Text("Elapsed Time: %02d:%02d:%02d.%03d", hours, minutes, seconds,
              milliseconds);
  ImGui::Text("Delta Time: %.0f µs",
              m_ctx.frameTimer->getDeltaTime() * 1000000);
  ImGui::Text("Status: %s", m_run ? "Running" : "Stopped");
  renderTimings(m_ctx.frameTimer->getTimings());
  ImGui::End();
}

void Primary::renderTimings(
...
void Primary::stressCarpet() {
  ...
  constexpr int N = 3;            // Grid width/height
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j < N; ++j) {
      bool isPinned =
          (i == 0 && j == 0) || (i == 0 && j == N - 1); // Pin two top corners
      bodies[i][j] =
          builder.createBody()
