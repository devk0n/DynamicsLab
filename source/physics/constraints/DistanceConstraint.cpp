...
void DistanceConstraint::computePositionError(VectorXd &phi, int startRow) const {
...
void DistanceConstraint::computeJacobian(MatrixXd &jacobian, int startRow) const {
  const auto &r1 = m_bodyA->getPosition();
  const auto &r2 = m_bodyB->getPosition();
  const auto &A1 = quaternionToRotationMatrix(m_bodyA->getOrientation());
  const auto &A2 = quaternionToRotationMatrix(m_bodyB->getOrientation());
...
void DistanceConstraint::computeAccelerationCorrection(VectorXd &gamma, int startRow) const {
  const auto &r1 = m_bodyA->getPosition();
  const auto &r2 = m_bodyB->getPosition();
